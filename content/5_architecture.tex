...
\myfigure{retrieval_schema.png}{Retrieval der relevanten Segmente}{retrieval_schema}
...
Zum Beispiel liefert die Suchanfrage \qt{Geschichte Amsterdam} mit dem TF-IDF Ansatz Sätze wie \qt{Amsterdam, das bedeutet unbeschwerte Kinderjahre} oder \qt{Amsterdam gefiel ihm}.
...
Wenn eine Person sich beispielsweise über die \qt{Geschichte von Amsterdam} informieren will, kommen dazu verschiedene Ausschnitte aus der Zeit der Gründung der Niederlande in 1581, den Besuchen Peter des Großen in Amsterdam im Jahr 1697 oder der Verfolgung der Juden im Zweiten Weltkrieg.
...
Zum Beispiel würde zu dem Thema \qt{Mauerfall Berlin} mehrere Segmente vorkommen, die sich auch mit der DDR befassen.
...
Wenn zum Beispiel in einem Podcast zur \qt{Geschichte von Amsterdam} einzelne Segmente über die Anfangszeit der Niederlande handeln, könnte man sich in einer weiterführenen Episode zum Beispiel über die \qt{Gründung der Niederlande} informieren.
...
Die Themen werden explizit auf 1-3 Wörter beschränkt, da sonst manchmal ganze Sätze als Themenvorschläge geführt werden, wie zum Beispiel \qt{Die Erfahrungen von Juden in Amsterdam während der Nazibesetzung}.
...
Als Parameter akzeptiert die API einen String \qt{text}, welcher die Frage des Nutzers angibt.
Außerdem kann über den Parameter \qt{time} ein Integerwert angegeben werden, welcher die Zeit des Podcasts in Minuten angibt.
Der optionale Parameter \qt{segment-length} kann als Integer übergeben werden und bestimmt die Länge der einzelnen Segmente.

Die API gibt bei erfolgreicher Verarbeitung den Statuscode 200 zurück und liefert als Inhalt ein JSON String zurück der eine einzige Variable \qt{url} enthält.
